...
WITH covered_by_selection AS (
    ...
    INNER JOIN {{ ref("dim_provinces") }} AS dim_prov
        ON st_covers(dim_prov.province_geometry, dim_mun.geom)
),
ordered_by_diff_area AS (
    SELECT
        id       AS municipality_id,
        statnaam AS municipality_name,
        geom     AS municipality_geometry,
        dim_prov.province_sk
    FROM st_read({{ source("geojson_external", "nl_municipalities") }}) AS dim_mun, {{ ref("dim_provinces") }} AS dim_prov
    WHERE NOT EXISTS (
            SELECT 1
            FROM covered_by_selection
...
SELECT
    ...
    src.*,
    {{ common_columns() }}
FROM ordered_by_diff_area AS src
UNION
SELECT
